...
def calculate_individual_score_relaxed(all_predicted, all_true, b_tag_idx, i_tag_idx):
    precision = []
    recall = []
    ...
    for seq in range(len(all_true)):
        ...
        fn = 0
        for tag in range(len(all_true[seq])):
            if (all_predicted[seq][tag] == b_tag_idx or all_predicted[seq][tag] == i_tag_idx) and (all_true[seq][tag] == b_tag_idx or all_true[seq][tag] == i_tag_idx):
                tp += 1
            elif (all_predicted[seq][tag] == b_tag_idx or all_predicted[seq][tag] == i_tag_idx) and (all_true[seq][tag] != b_tag_idx or all_true[seq][tag] != i_tag_idx):
                fp += 1
            elif (all_predicted[seq][tag] != b_tag_idx or all_predicted[seq][tag] != i_tag_idx) and (all_true[seq][tag] == b_tag_idx or all_true[seq][tag] == i_tag_idx):
                fn += 1
...
def calculate_average_score_relaxed(all_predicted, all_true, b_tag_idx, i_tag_idx):
    tp = 0
    fp = 0
    fn = 0
    for seq in range(len(all_true)):      
...
def print_scores_relaxed(all_predicted, all_true, tag2idx):
    org_precision, org_recall, org_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-ORG'], tag2idx['I-ORG'])
    print('org precision: %.4f    org recall: %.4f    org_f1: %.4f' %(org_precision.mean(), org_recall.mean(), org_f1))

    per_precision, per_recall, per_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-PER'], tag2idx['I-PER'])
    print('per precision: %.4f    per recall: %.4f    per_f1: %.4f' %(per_precision.mean(), per_recall.mean(), per_f1))

    loc_precision, loc_recall, loc_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-LOC'], tag2idx['I-LOC'])
    print('loc precision: %.4f    loc recall: %.4f    loc_f1: %.4f' %(loc_precision.mean(), loc_recall.mean(), loc_f1))

    date_precision, date_recall, date_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-DATE'], tag2idx['I-DATE'])
    print('date precision: %.4f    date recall: %.4f    date_f1: %.4f' %(date_precision.mean(), date_recall.mean(), date_f1))

    pro_precision, pro_recall, pro_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-PRO'], tag2idx['I-PRO'])
    print('pro precision: %.4f    pro recall: %.4f    pro_f1: %.4f' %(pro_precision.mean(), pro_recall.mean(), pro_f1))

    event_precision, event_recall, event_f1 = calculate_individual_score_relaxed(all_predicted, all_true, tag2idx['B-EVENT'], tag2idx['I-EVENT'])
    print('event precision: %.4f    event recall: %.4f    event_f1: %.4f' %(event_precision.mean(), event_recall.mean(), event_f1))

    org_tp, org_fp, org_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-ORG'], tag2idx['I-ORG'])
    per_tp, per_fp, per_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-PER'], tag2idx['I-PER'])
    loc_tp, loc_fp, loc_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-LOC'], tag2idx['I-LOC'])
    date_tp, date_fp, date_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-DATE'], tag2idx['I-DATE'])
    pro_tp, pro_fp, pro_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-PRO'], tag2idx['I-PRO'])
    event_tp, event_fp, event_fn = calculate_average_score_relaxed(all_predicted, all_true, tag2idx['B-EVENT'], tag2idx['I-EVENT'])
