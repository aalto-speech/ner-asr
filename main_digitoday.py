...
if __name__ == '__main__':
    ...
    whole_data_morph_path = 'utils/subword_segmentation/output/segmented/digitoday_segmented.txt'
    ...
    weights_matrix = np.load('weights/embedding_weights_digitoday.npy')
    ...
    if skip_training == False:
        ...
        model.load_state_dict(torch.load('weights/model_digitoday.pt'))
    ...
    np.save('plotting/digitoday_true.npy', all_true)
    np.save('plotting/digitoday_predict.npy', all_predicted)
    ...
    all_predicted, all_true = evaluate.get_predictions(data_wiki, model, word_num_layers, char_num_layers, morph_num_layers, word_hidden_size, 
                                                        char_hidden_size, morph_hidden_size, batch_size, device)
    evaluate.print_scores(all_predicted, all_true, tag2idx)
    np.save('plotting/wiki_true.npy', all_true)
    np.save('plotting/wiki_predict.npy', all_predicted)